# -*- coding: utf-8 -*-
"""
    @auther :  Doddahulugappa B
    Python Supported Version: Python 2.7.14
    Usage: python blockchain.py
    Description: Its a Simple Blockchain POC where each block points
    to the previous block. It's a basic Implementation and doesn't include advanced features like proof of work or peer to peer.
"""
...
class Message:
    def __init__(self, data):
        self.hash = None
        self.prev_hash = None
        self.timestamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')#time.time()
...
class Block:
    ...
    def link(self, block):
        """
            The block hash only incorporates the head message hash which then transitively includes all prior hashes.
        """
...
def manager():
    ...
    while True:
        print("---------------------------------------------------------------------------------")
        decide = raw_input("Enter Your action: ")
        if decide == "1":
            block.add_message(Message(raw_input("Enter Your Data:")))
        elif decide == "2":
            ...
            index = int(raw_input("Provide the index: "))
            ...
            print("Program Exited:User Either chosen Exit or Entered Invalid choice")
            break
